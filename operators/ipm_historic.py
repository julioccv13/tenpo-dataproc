...
class Ipm_historic:
	...
	def write_files_gcs(self):
		...
		move_files = "gsutil -m cp -r ./ipm_csv/* {0}IPM/{1}/".format(self.output,self.timestamp_bucket)
		os.system(move_files)

	def write_files_bigquery(self):
		for name_file in self.list_file :
			name_csv = name_file.replace("_encrypted","")
			put_csv_to_hdfs = "hdfs dfs -put ./ipm_csv/{0}.csv /".format(name_csv)
			os.system(put_csv_to_hdfs)
			df = self.spark.read.options(header='True',delimiter=',',encoding='UTF-16').csv("hdfs:///"+name_csv+".csv")
			df.printSchema()
			...
			df1 = df.select([regexp_replace(col(c), "�", "").alias(c) for c in df.columns]).withColumn("NAME_FILE_ENTIRE", input_file_name()).withColumn("NAME_FILE",regexp_replace("NAME_FILE_ENTIRE","hdfs://"+cluster_name+"-m/","")).withColumn("ID_ROW", udf(lambda : str(uuid4()), StringType())()).withColumn("DATE_PROCESS_FILE",lit(self.timestamp)).withColumn('FECPROCES',to_timestamp( regexp_replace(concat(lit("20"),substring("NAME_FILE",25,14)),".T"," " ) ,"yyyyMMdd HHmmss" ))
